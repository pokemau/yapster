...
// Function to add users to the group chat
function addUsersToGroupChat(chatId, userIds) {
    fetch(`/chat/${chatId}/add_members/`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        ...
        .then(data => {
            if (data.success) {
                alert("Members added successfully!");
                // Optionally, you can refresh the chat view or update UI elements
                toggleAddMemberState(); // Close the Add Member state
            } else {
                alert("Failed to add members.");
            }
        })
